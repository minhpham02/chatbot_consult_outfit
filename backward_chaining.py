            ...
            if rule.right==goal:
                ...
                for f in range(len(rule.left)):
                    ...
                    if fact_guess not in self.target_facts:
                        self.print_step(fact_guess, indent + '-', "Không có luật nào để suy diễn/không có đặc điểm này ban đầu. Trả về thất bại.")
                        dk=0
                        break
                    else:
                        self.print_step(fact_guess, indent +"-", "Cập nhật  %s, bởi vì được tìm thấy trong tập gốc %s. Trả về thành công." % (
